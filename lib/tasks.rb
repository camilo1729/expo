...
class Task
  ...
  include Observable
  attr_accessor :name, :options, :dependency, :target, :cloned_from, :executable, :resource, :cloned
  attr_reader :exec_part, :children, :sync, :start_time, :end_time
  ...
  def initialize(name, options ={}, &block)
    ...
    @cloned_from = nil
    @children = []
    @executable = options[:res_granularity].nil? ? true : false 
    #it is executable when the task is synchronous from the begining
    @start_time = nil
    @end_time = nil
  end
